...
(try
  ...
  (let [login-result (wa/login)]
    (println "Login function returned:" login-result)
  ...
  ;; Wait a bit for login to complete after scan/initial check
  (Thread/sleep 5000)
  ...
  (catch Exception e
    ...
    (.printStackTrace e)
    (println "Make sure the pod executable './bb-whatsapp-pod' exists and has execute permissions.")))

;; Ensure pod process terminates
(shutdown-agents) 